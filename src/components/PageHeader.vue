  ...
  <div class="etchHead">
    <div class="main mw">
      ...
      <div class="navigation">
        <router-link active-class="active-menu" :to="{ path: '/bscsMarket' }"
          >Home</router-link
        >
        <router-link active-class="active-menu" :to="{ path: '/Tokens' }"
          >Tokens</router-link
        >
        <router-link active-class="active-menu" :to="{ path: '/Marketplace' }"
          >Marketplace</router-link
        >
        <router-link active-class="active-menu" :to="{ path: '/Runes' }"
          >Runes</router-link
        >

        <!--
            <router-link active-class="active-menu" :to="{ path: '/Deploymint' }"
              >Deploy
            </router-link>
        -->
      </div>
      <div>
        <div class="etchRight" v-show="!address" @click="connectDialog = true">
          Connect<span>Wallet</span>
        </div>

        <el-dropdown
          popper-class="login_drop"
          v-show="address"
          trigger="click"
          @command="doneCommand"
        >
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item :icon="House" :command="1"
                >All Bscscriptions</el-dropdown-item
              >
              <el-dropdown-item :icon="DocumentCopy" :command="2"
                >Copy Address</el-dropdown-item
              >
              <el-dropdown-item :icon="SwitchButton" :command="3"
      ...
      <div class="body">
        <div class="header">
          <div class="left">
            ...
            <div class="cart_clear" v-if="tickLen > 0" @click="clearTicks">
              Clear All
            </div>
          </div>
        ...
        <div v-if="tickLen === 0" class="empty">
          ...
          <el-button class="btn" @click="doneCommand(4)" round
            >Explore Collections</el-button
          >
        </div>
                ...
                <template #default="scope">
                  <div class="tick_item">
                    <div class="tick_icon">
                      <el-icon
                        class="close"
                        @click="delTick(scope.row.order_id)"
                      >
                        <ElClose />
                      </el-icon>
                    </div>
            ...
            <div class="num">
              ...
              <div>
                <img
                  ...
                  src="@/assets/images/icon_bnb.png"
                  alt=""
                />
      ...
      <el-row :gutter="20" align="middle">
        ...
        <el-col :span="12">
          <div class="right_main">
            ...
            <img
              class="icon"
              src="@/assets/images/connect_right_icon.png"
              alt=""
            />
  ...
  if (!walletObj) {
    ...
    if (type) {
      ElMessageBox.confirm(
        "Not found the " + key + " connect plugins",
        "Connect Wallet",
        {
          type: "warning",
...
async function getList() {
  ...
  list.value = (result.data || []).map((item) => {
    ...
    let amount =
        "" + Math.max(Number(item.order_amt || 0) - Number(item.buy_lim), 0),
...
async function buyBatch() {
  ...
  let signDatas = await Promise.all(
    keys.map((key) => {
  ...
  let abi = that.$Contract.control.methods
    .batchBuy(ords, pids, orderAmts, buyAmts, prices, sells, deadlines, signs)
    .encodeABI();
  let res = await that.$sendTransition(
    that.$Contract.control._address,
    abi,
    bnb.toString()
  );
  if (res.status) {
      ...
      .table_wrap {
        ...
        .tick_item {
          ...
          .tick_icon {
            ...
            z-index: 9;
            margin: auto;
            background: url("@/assets/images/bscs.jpg") center/125% 125%
              no-repeat;
...
.el-overlay .connect_dialog {
  ...
  .el-dialog__body {
    ...
    .right_main {
      ...
      justify-content: center;
      flex-direction: column;
      background: url("@/assets/images/connect_right_bg.png") center / 80% auto
        no-repeat;
